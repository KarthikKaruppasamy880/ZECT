/**
 * Electron a11y/UX smoke. Skip if electron.exe is missing — skip ≠ PASS.
 */
...
const ART = path.join(REPO, "test-results", "ux-accessibility-electron");
...
test.describe("ux accessibility electron", () => {
  test.setTimeout(180_000);

  test("Companion, Present, Developer, Knowledge via sidebar", async () => {
    ...
    try {
      ...
      await page.screenshot({ path: path.join(ART, "02-present.png") });

      await nav.getByRole("link", { name: "Developer" }).click();
      ...
      await nav.getByRole("link", { name: "Knowledge Base" }).click();
      await expect(page.getByTestId("knowledge-base-page")).toBeVisible({ timeout: 30_000 });
      await page.screenshot({ path: path.join(ART, "04-knowledge.png") });
    } finally {
      await app.close();
    }
  });
