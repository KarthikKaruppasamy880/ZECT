...
import {
  ...
  toggleSchedule, triggerSchedule, getPlaybooks,
} from "@/lib/api";

const TASK_TYPES = ["review", "build", "deploy", "report", "custom", "mentrix"];
...
export default function ScheduledTasks() {
  ...
  const [form, setForm] = useState({
    ...
    interval_minutes: 60,
    task_type: "review",
    playbook_id: "" as string | number,
    task_config: { owner: "", repo: "", branch: "main", goal: "" },
  });
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const [res, pbs] = await Promise.all([getSchedules(), getPlaybooks()]);
      setSchedules(res.items || []);
      setTotal(res.total || 0);
      setPlaybooks(pbs.items || []);
    } catch (e: any) {
  ...
  const validateTarget = () => {
    const hasPlaybook = form.playbook_id !== "" && form.playbook_id != null;
    const hasTask = Boolean(form.task_type);
    if (!hasPlaybook && !hasTask) {
      setError("Choose a task type or a playbook (at least one is required).");
      return false;
    }
    return true;
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      const playbookId =
        form.playbook_id === "" || form.playbook_id == null
          ? undefined
          : Number(form.playbook_id);
      await createSchedule({
        ...
        task_type: form.task_type || (playbookId ? "custom" : "review"),
  ...
  const startEdit = (s: any) => {
    ...
    setForm({
      ...
      task_type: s.task_type || "review",
      playbook_id: s.playbook_id ?? "",
      task_config: s.task_config || { owner: "", repo: "", branch: "main", goal: "" },
    });
    setShowCreate(false);
  };

  return (
    <div className="max-w-6xl mx-auto space-y-6" data-testid="scheduled-tasks-page">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            {total} schedules — cron/interval Mentrix or playbook runs. Wire{" "}
            <code className="text-xs bg-slate-100 px-1 rounded">POST /api/schedules/due/run</code> for
            automatic due execution.
          </p>
        </div>
        <button
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-4 shadow-sm">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <select
              ...
              className="px-4 py-2 border border-slate-300 rounded-lg text-sm"
              data-testid="schedule-task-type"
            >
              {TASK_TYPES.map(t => (
          ...
          <div>
            <label className="text-xs font-medium text-slate-600">Playbook (optional — preferred when set)</label>
            <select
              value={form.playbook_id === "" ? "" : String(form.playbook_id)}
              onChange={e =>
                setForm({
                  ...form,
                  playbook_id: e.target.value === "" ? "" : Number(e.target.value),
                })
              }
              className="mt-1 w-full px-4 py-2 border border-slate-300 rounded-lg text-sm"
              data-testid="schedule-playbook"
            >
              <option value="">— Mentrix task type only —</option>
              {playbooks.map((pb: any) => (
                <option key={pb.id} value={pb.id}>{pb.name}</option>
              ))}
            </select>
            <p className="mt-1 text-xs text-slate-500">Require task type and/or playbook before save.</p>
          </div>
          <textarea
            placeholder="Description / Mentrix goal (optional)"
