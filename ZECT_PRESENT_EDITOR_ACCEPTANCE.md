...
- Canvas is a `PresentationDocument` compositor (`present-editor-canvas` `data-canvas="document"`). Thumbs use the same component. PowerPoint COM / LibreOffice rasters are fidelity oracles, not the editor source of truth.
- Overlay uses EMU geometry from `p:sp` / `p:pic` / `graphicFrame` plus locked master/layout graphics. Fake 2-column CHART/SHAPE grids are not the normal editor.
