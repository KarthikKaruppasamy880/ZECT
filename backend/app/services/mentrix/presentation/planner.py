...
_SYSTEM = (
    ...
    "Each slides[] item: purpose, title (short headline, max ~12 words, not the full topic sentence), "
    "key_message (one supporting sentence that expands the title — never duplicate the title verbatim), "
    "content_blocks[{kind,text}] (3-5 supporting points), "
    "evidence[{source_type,source_id,excerpt}], "
    "visual_intent (none|chart|table|image|quote|metric|diagram|comparison|timeline|process|architecture|card_grid), "
    ...
    "When a slide explains 2-4 parallel named concepts (e.g. 'three modes', 'four pillars', distinct "
    "capabilities/options/types) put ALL of them on that ONE slide with visual_intent 'card_grid' and one "
    "content_blocks entry per concept, formatted 'Name - short description' — never split parallel concepts "
    "across separate slides, and never put more than one concept's name in a single content_blocks entry. "
    ...
    "Quality bar (Gamma/Beautiful-AI-class, not a wall of text): coherent story with a clear arc, strong "
    "visual hierarchy, concise titles, LIMITED visible text density (short phrases over full sentences — "
    "content_blocks entries should read like headlines, not paragraphs), purposeful visuals chosen because "
    "they clarify the point (not decoration), generous whitespace, no filler, no hallucinated KPIs, "
    "professional executive readability. Match the narrative pattern to the deck type: an executive brief "
    "runs context → problem → evidence → recommendation → next steps; a technical deck runs problem → "
    "architecture → flow → implementation → risks; a training deck runs concept → example → process → "
    "practice → summary; a sales deck runs problem → opportunity → solution → proof → call to action."
)
