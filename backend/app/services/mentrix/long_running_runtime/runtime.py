...
class LongRunningAgentRuntime:
    ...
    def tick(
        ...
        *,
        ...
        for _ in range(max(1, int(max_ops))):
            ...
            manifest = store.read_json("EXECUTION_MANIFEST.json", default={}) or {}
            current_op = next((o for o in (manifest.get("operations") or []) if o.get("id") == op_id), None) or {}
            if str(current_op.get("status")).lower() in ("done", "completed", "verified"):
                processed.append(op_id)
                continue  # idempotent

            # Non-synthetic: apply real worktree mutation / coding agent when workspace present
            if not state.get("synthetic", True) and (row.worktree_path or "").strip():
                exec_out = self._execute_real_operation(
                    row=row,
                    op=current_op,
                    state=state,
                    worker_id=worker_id,
                )
                tel["actual_model"] = exec_out.get("model") or tel["actual_model"]
                tel["provider"] = exec_out.get("provider") or tel["provider"]
                tel["latency_ms"] = int(exec_out.get("latency_ms") or 0)
                telemetry[-1] = tel
                row.telemetry_json = json.dumps(telemetry)[:200_000]
                if not exec_out.get("ok"):
                    checkpoint, tripped = breaker.record(checkpoint, str(exec_out.get("error") or "op_failed"))
                    state["failure_signature"] = checkpoint.last_error
                    state["same_failure_count"] = checkpoint.same_failure_count
                    record_checkpoint(store, checkpoint_type="failure", operation_id=op_id, payload=exec_out)
                    self._persist_state(row, store, checkpoint, state, runtime_used)
                    self.db.commit()
                    if tripped:
                        return self._block(row, store, "circuit_breaker", checkpoint, state, runtime_used, tripped=True)
                    return {"ok": False, "error": exec_out.get("error"), "processed": processed, **self.serialize(row)}
                if exec_out.get("current_commit_sha"):
                    row.current_commit_sha = exec_out["current_commit_sha"]
    ...
    def _execute_real_operation(
        ...
        *,
        ...
        """Apply a bounded real worktree change for one operation (non-synthetic runs)."""
        import subprocess
        import time
        ...
        rel = str(op.get("path") or op.get("file") or f"lrr_{op.get('id', 'op').lower()}.py")
        ...
        content = str(
            op.get("content")
            or (
                f'"""Generated by Mentrix LRR op {op.get("id")} — disposable test only."""\n'
                f"OP_ID = {op.get('id')!r}\n"
                f"def ping():\n    return {op.get('id')!r}\n"
            )
        )
        ...
        # Prefer mentrix_native coding agent when configured; always record git state
        model = state.get("actual_model") or "filesystem"
        provider = "local"
        try:
            from app.adapters.coding_runtime import selected_coding_engine

            engine = selected_coding_engine()
            provider = "cloud" if engine == "mentrix_native" else engine
            model = f"{engine}:{state.get('actual_model') or 'default'}"
        except Exception:
            engine = "filesystem"
        sha = ""
        try:
            subprocess.run(["git", "add", "--", rel], cwd=str(root), check=False, capture_output=True)
            subprocess.run(
                ["git", "commit", "-m", f"lrr:{op.get('id')} {op.get('title') or rel}"],
                cwd=str(root),
                check=False,
                capture_output=True,
            )
            ...
            return {"ok": False, "error": f"git_failed:{exc}"[:300]}
        latency_ms = int((time.perf_counter() - t0) * 1000)
        record_checkpoint(
            ArtifactStore(row.work_item_id),
            checkpoint_type="file_change",
            operation_id=str(op.get("id")),
            payload={"path": rel, "worker_id": worker_id, "engine": engine},
            worktree_path=str(root),
            base_commit_sha=row.base_commit_sha or None,
            current_commit_sha=sha or None,
        )
        return {
            "ok": True,
            "path": rel,
