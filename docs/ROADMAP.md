...
| 3 | Cursor-like workspace | **In progress (Stage A)** | Stage A: `/workspace` shell with file tree + Monaco + git status/branch strip + path-scoped writes. See `PHASE_3_EXECUTION_PLAN.md`. Remaining: terminal, hunks, inline Ask, symbols/refs, worktree UI. |
...
**Phase 3 Stage A** is the current deliverable (unified Developer Workspace shell). Stop after each Stage merge and wait for approval before Stage B. Phase 9 remains ON HOLD.
