...
def engine_health(_user: CurrentUser = Depends(get_current_user)):
    ...
    body.update(resolve_isolation())
    return body


@router.post("/workspaces")
def create_workspace(req: ProvisionRequest, _user: CurrentUser = Depends(get_current_user)):
    from app.services.coding_engine.workspace import provision_isolated_workspace

    try:
        ws = provision_isolated_workspace(repo_path=req.repo_path, run_id=req.run_id)
    ...
    return {
        ...
        "sandbox_image": ws.sandbox_image,
        "isolation_note": ws.isolation_note,
...
def remove_workspace(req: DisposeRequest, _user: CurrentUser = Depends(get_current_user)):
    from app.services.coding_engine.workspace import dispose_isolated_workspace

    try:
        result = dispose_isolated_workspace(
            workspace_id=req.workspace_id,
            repo_path=req.repo_path,
            workspace_path=req.workspace_path,
            container_id=getattr(req, "container_id", None),
